...
def test_head_only(supervised_model1, supervised_model2, supervised_model3, dataloader, folder_path, num, target, device: str = 'cpu'):
    ...
    with torch.no_grad():
        ...
        # tqdm을 사용하여 진행 상황 표시
        for idx, (inh, rgh, rgv, masks, y_true, file_name, inputs_org) in tqdm(enumerate(dataloader), total=len(dataloader), desc=f"[{num}] Testing"):
            inh, rgh, rgv, masks = inh.to(device), rgh.to(device), rgv.to(device), masks.to(device)
            y_true_list.append(y_true.item())

            zero = np.zeros(inh.shape)
            ...
            final_outputs = total_output
            final_segmap = total_output.cpu().detach().numpy()
            final_segmap = final_segmap.squeeze()

            img = cv2.cvtColor(inh.permute(0, 2, 3, 1).cpu().numpy()[0] * 255, cv2.COLOR_RGB2GRAY)
...
def test_skirt(supervised_model, dataloader, folder_path, num, target, device: str = 'cpu'):
    ...
    with torch.no_grad():
        ...
        # tqdm을 사용하여 진행 상황 표시
        for idx, (img, masks, y_true, file_name, inputs_org) in tqdm(enumerate(dataloader), total=len(dataloader), desc=f"[{num}] Testing"):
            ...
            # supervised predict
            s_output = supervised_model(img).to(device)
            total_output = F.sigmoid(s_output)
            
            final_outputs = total_output
            final_segmap = total_output.cpu().detach().numpy()
            final_segmap = final_segmap.squeeze()

            img = cv2.cvtColor(img.permute(0, 2, 3, 1).cpu().numpy()[0] * 255, cv2.COLOR_RGB2GRAY)
            img = np.uint8(min_max_norm(img) * 255)
            inputs_org = np.uint8(inputs_org).squeeze()
...
def test_individual(supervised_model, dataloader, folder_path, num, target, device: str = 'cpu'):
    supervised_model.eval()
    y_true_list = []
    y_pred_list = []
    prob_list = []
