...
class AttentiveInputTransform(object):
    ...
    def __call__(self, image, mask):
        ...
        for idx in rand_indices:
            img, target = self.dataset[idx]  # Adjust based on your dataset structure
            rand_img.append(img)
            rand_target.append(target)

        rand_img = torch.stack(rand_img)
        rand_target = torch.stack(rand_target)
        # print('rand_target',rand_target.shape)
        ...
        rand_img,rand_target = self._replace_attentive_regions(rand_img, image,rand_target, mask, attentive_regions)

        # return rand_img.resize(ori_size)
        return rand_img.to('cuda'),rand_target.to('cuda')
    
    def _replace_attentive_regions(self,rand_img, image,rand_mask, mask, attentive_regions):
        """
        ...
        for attentive_region in attentive_regions:
            ...
            self._replace_attentive_region(rand_img, image,rand_mask, mask, attentive_region)
        # return Image.fromarray(np_rand_img)
        return rand_img,rand_mask

    def _replace_attentive_region(self, np_rand_img, np_img, np_rand_mask, np_mask, attentive_region):
