...
from train import training_supervise, training_iter_supervise, write_to_memory_mapped_file
...
def run_training(cfg):
    ...
    # Transfer Learning
    if cfg['Train']['transfer_learning_dir'] is not None:
...
if __name__=='__main__':
    ...
    config = ConfigParser()
    # D:\DeepLearningStudio\common\bin\x64\config\land_config.ini
    exe_path = os.path.dirname(os.path.abspath(__file__))  # 현재 스크립트의 디렉토리 경로
    parent_path = os.path.abspath(os.path.join(exe_path, "../../../../"))
    file_path = os.path.join(parent_path, "common", "bin", "x64", "config", "land_config.ini")
    _logger.info('train')
    ...
    # pyinstaller .\main_land_train_supervise.py   ("../../")