...
class ResNet(nn.Module):
    ...
    def _forward_impl(self, x: Tensor) -> Tensor:
        ...
        # torch.save(feature_c,"python_x.pt")

        feature_d = self.layer4(feature_c)


        return [act1, feature_a, feature_b, feature_c,feature_d]
...
def _resnet(
    ...
    **kwargs: Any
    ...
    if pretrained:
        ...
        # torch.save(state_dict,'python_resnet.pth')

        # example = torch.rand(1, 3, 1024, 1024)
        # # ex = example.cuda()
        # tt = torch.jit.trace(model, example)
        # tt.save('resnet_pretrained weight/resnet_pretrained_weight.pth')
...
class BN_layer(nn.Module):
    ...
    def _forward_impl(self, x1: Tensor,x2: Tensor,x3: Tensor) -> Tensor:
        # See note [TorchScript super()]
        #x = self.cbam(x)
        # t1 = torch.rand(1, 256, 256, 256)
        # t2 = torch.rand(1, 512, 128, 128)
        # t3 = torch.rand(1, 1024, 64, 64)
        #
        # l1 = self.relu(self.bn2(self.conv2(self.relu(self.bn1(self.conv1(t1))))))
        # l2 = self.relu(self.bn3(self.conv3(t2)))
        # feature = torch.cat([l1,l2,t3],1)
        ...
        output = self.bn_layer(feature)
        # torch.save(output,"python_x.pt")


        return output.contiguous()

    def forward(self, x1: Tensor,x2: Tensor,x3: Tensor) -> Tensor:
        return self._forward_impl(x1,x2,x3)
